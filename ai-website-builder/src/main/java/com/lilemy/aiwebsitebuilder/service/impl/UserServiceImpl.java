...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public long userRegister(String userAccount, String userPassword, String checkPassword) {
        // 1. 校验
        ThrowUtils.throwIf(StrUtil.hasBlank(userAccount, userPassword, checkPassword), ResultCode.PARAMS_ERROR, "参数为空");
        ...
        ThrowUtils.throwIf(!saveResult, ResultCode.SYSTEM_ERROR, "注册失败，数据库错误");
        return user.getId();
    }
    ...
    public LoginUserVO userLogin(String userAccount, String userPassword) {
        // 1. 校验
        ThrowUtils.throwIf(StrUtil.hasBlank(userAccount, userPassword), ResultCode.PARAMS_ERROR, "参数为空");
        ThrowUtils.throwIf(userAccount.length() < 4, ResultCode.PARAMS_ERROR, "账号错误");
        ThrowUtils.throwIf(userPassword.length() < 8, ResultCode.PARAMS_ERROR, "密码错误");
        // 2. 加密
        String encryptPassword = getEncryptPassword(userPassword);
        // 查询用户是否存在
        QueryWrapper queryWrapper = new QueryWrapper();
        ...
        // 用户不存在
        ThrowUtils.throwIf(user == null, ResultCode.PARAMS_ERROR, "用户不存在或密码错误");
        // 3. 记录用户的登录态
